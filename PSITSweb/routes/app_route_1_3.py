    ...
    for data in dataset:
        ...
        if order.getStatus() == ORDER_STATUS.ORDERED.value:
            ...
            if key_format in montly_orders:
                ...
                montly_orders[key_format] = orders + \
                    (GetPriceRef(order.reference) * order.quantity)
            else:
                # add a new entry to the monthly orders
                montly_orders[key_format] = (
                    GetPriceRef(order.reference) * order.quantity)
            ...
            if key_format in monthly_revenue:
                ...
                monthly_revenue[key_format] = revenue + \
                    (GetPriceRef(order.reference) * order.quantity)
            else:
                # add a new entry to the monthly revenue
                monthly_revenue[key_format] = (
                    GetPriceRef(order.reference) * order.quantity)
